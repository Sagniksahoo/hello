...
def copy_generated_files_to_repo(generated_path, repo_path):
    """Copy all files and directories from the generated folder to the cloned repository."""
    try:
        for item in os.listdir(generated_path):
            src = os.path.join(generated_path, item)
