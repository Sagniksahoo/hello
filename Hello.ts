generateJSON(): Blob {
  ...
  const jsonData = {
    ...
    resources: this.addedResources.map(resource => {
      // Define the object to hold parameter key-value pairs
      const parametersObject: { [key: string]: string } = {};

      // Map parameters into the object
      resource.parameters.forEach(param => {
        parametersObject[param.label] = param.type === 'checkbox'
          ? this.getCheckboxValues(param) 
          : param.value || ''; // Assign the actual value
      });
